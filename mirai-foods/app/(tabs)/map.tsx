...
export default function MapScreen() {
  ...
  const renderProductPin =
    (numColumns = 2) =>
    ({ item }: { item: Product }) => {
      const hours = Math.floor(item.expiringInHours);
      const minutes = Math.max(
        0,
        Math.round((item.expiringInHours % 1) * 60)
      );
      return (
        <TouchableOpacity
          style={[
        ...
        >
          ...
          >
            ...
            <ThemedText
              style={[styles.mostOrderedMeta, styles.discountBadgeText]}
            >
  ...
  const dataSource = selectedPlace
    ? [...selectedPlace.products].sort(
        (a, b) => b.discountPercentage - a.discountPercentage
      )
    : places
        .flatMap((place) => place.products)
      ...
      <View style={styles.headerSection}>
        ...
        {selectedPlace ? (
          ...
          <View style={styles.noPlaceSelected}>
            <ThemedText style={styles.subtitle}>
              Tap a bakery marker to see products (sorted by highest
              discount).
            </ThemedText>
          </View>
        )}
    ...
    >
      <FlatList
        ...
        contentContainerStyle={[
          styles.fullWidthList,
          { paddingBottom: insets.bottom + 160, paddingHorizontal: 16 },
        ]}
